...
function Strategy(o){
    ...
    self._sessionTokenField = opts._sessionTokenField || 'sessionToken';
...
Strategy.prototype.authenticate = function(req, options) {
    ...
    var token = lookup(req.body, this._sessionTokenField) || lookup(req.query, this._sessionTokenField);
    var username = lookup(req.body, this._usernameField) || lookup(req.query, this._usernameField);
    var password = lookup(req.body, this._passwordField) || lookup(req.query, this._passwordField);

    if (token) {
        self.parseClient.User.enableUnsafeCurrentUser();
        self.parseClient.User.become(token, {
          success: function(user){
            return self.success(user);
          },
          error: function(user, error){
            return self.fail({message: 'Invalid credentials'}, 400);
          }
        });
    }
      ...
      return self.fail({ message: options.badRequestMessage || 'Missing credentials (either session token or username/password' }, 400);
    }
};
